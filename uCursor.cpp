...
//---------------------------------------------------------------------------
CIter::CIter(TSHEdit * component, const Iter& itr, short type, bool myown)
: itr(itr), component(component), type(type), myown(myown)
{
}
//---------------------------------------------------------------------------
CIter::CIter(const CIter& c)
: itr(c.itr), component(c.component), type(c.type), myown(myown)
{
}
//---------------------------------------------------------------------------
void CIter::Init()
...
void CIter::Update()
{
  if(!myown)
  {
    assert((type & CITER_BOTH) != 0);

    if((CITER_BOTH & type) == CITER_BOTH)
      component->SetSelection(&itr,&itr);
    else if (CITER_END & type)
      component->SetSelection(&component->itrCursor,&itr);
    else if (CITER_BEGIN & type)
      component->SetSelection(&itr, &component->itrCursorSecond);
  }
}
//---------------------------------------------------------------------------
CIter::~CIter()
...
/*AUTOINCLUDE_BODY_CIter_uIter.h*/
      ...
      /*AUTOGENERATED*/ wchar_t  CIter::GetPrevChar ()  { Init();  return  this->itr.GetPrevChar ();} 
      ...
      /*AUTOGENERATED*/ bool  CIter::LineIsEmpty ( bool allowWhite )  { Init();  return  this->itr.LineIsEmpty (allowWhite);} 
