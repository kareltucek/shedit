...
bool Parser::ParserState::operator==(const ParserState& state)
{
  for(int i = 0; i < bankCount; i++)
    if(!(searchStateBank[i] == state.searchStateBank[i]))
      return false;
    return (this->parseid == state.parseid &&  markupStack == state.markupStack && actBank == state.actBank);
}
//---------------------------------------------------------------------------
bool Parser::ParserState::operator!=(const ParserState& state)
{
...
proc:
        switch(type)
        {
            ...
            case LangDefSpecType::PushPop:
                if(searchiter->mask & searchiter->current->popmask)
                {
...
{
  int popmask = sit->mask & sit->current->popmask;
...
//---------------------------------------------------------------------------
void Parser::PerformJump(LanguageDefinition::SearchIter *& sit)
{
/*
